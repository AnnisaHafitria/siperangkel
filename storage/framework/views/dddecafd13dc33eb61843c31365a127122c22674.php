    ...
    <div class="card">
      <h2 class="card-header">
        ...
        <div class="float-right">
          <a class="btn btn-xs btn-outline-dark badge-pill" href="<?php echo e(route('pengembalian.index')); ?>"><i class="zmdi zmdi-close-circle"></i> Batal</a>
        </div>
      </h2>
      <div class="card-body">
        <form action="<?php echo e(route('pengembalian.store')); ?>" method="post" id="insert_form">
            ...
            <center>
              <table id="item_table" class="table table-bordered">
                <tr id="last" align="center">
                  ...
                  <th style="background-color:#2196F3"><font color="white">Tanggal Peminjaman</th>
                  <th style="background-color:#2196F3"><font color="white">Batas Waktu Peminjaman</th>
                  <th style="background-color:#2196F3"><font color="white">Tanggal Pengembaliann</th>
                  <th style="background-color:#2196F3"><font color="white">Denda</th>
                  <th style="background-color:#2196F3"><font color="white">Keterangan</th>
                  <th><button type="button" name="add" class="btn btn-outline-primary btn-xs badge-pill add" onclick="addrow()"><i class="zmdi zmdi-plus-square"></i></button></th>
                </tr>
              </table>
            </center>
            <br>
            <div align="left">
              <button type="submit" name="submit" class="btn btn-outline-success btn-xs badge-pill" value="Kembalikan"><i class="zmdi zmdi-caret-left-circle"></i> Kembalikan</button>
            </div>
          </div>
        </form>
      </div>
...
<script>
  //multiple insert
    function addrow(){
      ...
      html +='<td><input type="date" name="tanggal_pinjam[]" class="form-control date"/></td>';
      html +='<td><input type="date" name="tanggal_batas[]" class="form-control date"/></td>';
      html +='<td><input type="date" name="tanggal_kembali[]" class="form-control date"/></td>';
      html +='<td><input type="text" name="denda[]" class="form-control"/></td>';
      html +='<td><input type="text" name="keterangan[]" class="form-control ="/></td>';
      html +='<td><button type="button" class="btn btn-outline-danger btn-xs badge-pill" onclick="remove('+ no +')"><i class="zmdi zmdi-minus-square"></i></button></td></tr>';
      $('#last').after(html);
      $('.sl2').select2();
      $('.date').flatpickr({
