    ...
    <div class="card">
      <h2 class="card-header">
        ...
        <div class="float-right">
          <a class="btn btn-xs btn-outline-dark badge-pill" href="{{route('tentang_program.index')}}"><i class="zmdi zmdi-close-circle"></i> Batal</a>
        </div>
      </h2>
      <div class="card-body">
        <form action="{{ route('tentang_program.update',$tentang_program->id) }}" method="post">
          <input name="_method" type="hidden" value="PATCH">
          {{csrf_field()}}
          <div class="form-group">
            <label class="control-label"><font style="color:grey">Judul</label> 
            <input type="text" name="judul" value="{{ $tentang_program->judul }}" class="form-control" required>
          </div>
          <div class="form-group">
            <label class="control-label"><font style="color:grey">Deskripsi</label> 
            <input type="text" name="deskripsi" value="{{ $tentang_program->deskripsi }}" class="form-control" required>
          </div>
          <div class="form-group">
